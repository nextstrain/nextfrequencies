...
export function makeColourScale(categories) {
  console.log("makeColourScale", categories);
  ...
  while (remaining>0) {
    if (remaining < colours.length-1) {
      hexes.push(...colours[remaining]);
      remaining = 0;
    } else {
      hexes.push(...colours[colours.length-1]);
      remaining -= (colours.length-1);
    }
  }
  console.log(hexes)
  const colorMap = {};
  categories.forEach((c, i) => {colorMap[c]=hexes[i];})

  return (grouping) => colorMap[grouping] || '#000';
